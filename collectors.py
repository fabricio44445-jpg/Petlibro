"""Live data collectors for Petlibro.
...
USER_AGENT = (
    "Mozilla/5.0 (compatible; Petlibro/1.0; "
...
PETLIBRO_DOMAIN_TERMS = {
    "feeder", "fountain", "automatic", "pet", "cat", "dog", "dispenser",
    "wi-fi", "wifi", "app", "granary", "dockstream", "polar", "rfid", "collar",
    "food", "water", "portion", "clogged", "battery",
}
...
def brand_is_relevant(brand: str, text: str) -> bool:
    ...
    if not re.search(rf"(?<!\w){phrase}(?!\w)", text, flags=re.IGNORECASE):
        return False
    if brand.casefold() == "petlibro":
        normalized = text.casefold()
        return any(term in normalized for term in PETLIBRO_DOMAIN_TERMS)
    return True
...
def sentiment_for(
    ...
    blob = TextBlob(f"{title}. {summary[:350]}").sentiment  # type: ignore[attr-defined]
    blob_score = float(getattr(blob, "polarity", 0.0))
...
def collect_mentions(
    ...
    for brand in brands:
        ...
        if "Google News" in sources:
            collectors.append(("Google News", lambda brand=brand: fetch_google_news(brand)))
        if "Reddit" in sources:
            collectors.append(("Reddit", lambda brand=brand: fetch_reddit(brand)))
        if "Blogs" in sources:
            collectors.append(("WordPress", lambda brand=brand: fetch_wordpress(brand)))
